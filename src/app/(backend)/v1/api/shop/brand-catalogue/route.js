...
export async function GET(request) {
    try {
        ...
        const totalDocs = await brand_catalogueSchema.find({ ...que }).countDocuments();
        const totalPages = Math.ceil(totalDocs / limit);
        const data = await brand_catalogueSchema.find({ ...que }).select(fieldSelector)
            .skip((page - 1) * limit)
            .limit(limit)
            .sort({ sort: 1 })
            .populate({ path: "brand",select: "name slug _id", match: { isActive: true } })
