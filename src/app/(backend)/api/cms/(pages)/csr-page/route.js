...
import { DbValidator } from "@/app/utils/db/schema_validation/csr-page.validation.js";
...
export async function GET(request) {
    try {
        ...
        let data = await csr_pageSchema.findOne()
...
export async function PUT(request) {
    ...
    try {
        ...
        let record = await csr_pageSchema.findOne();

        if (record) {
            await csr_pageSchema.findByIdAndUpdate(
                record._id,
            ...
            await csr_pageSchema.create({ ...objToPush });
            return NextResponse.json({
                success: true,
