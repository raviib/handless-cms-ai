...
import { DbValidator } from "@/app/utils/db/schema_validation/vigro-gold-club-page.validation.js";
...
const FILE_PATH = "/file/vigro-gold-club-page";
...
export async function GET(request) {
    try {
        ...
        let data = await vigro_gold_club_pageSchema.findOne()
            .select(fieldSelector); // Populate all reference fields
...
export async function PUT(request) {
    ...
    try {
        ...
        let record = await vigro_gold_club_pageSchema.findOne();

        if (record) {
            await vigro_gold_club_pageSchema.findByIdAndUpdate(
                record._id,
            ...
            await vigro_gold_club_pageSchema.create({ ...objToPush });
            return NextResponse.json({
                success: true,
