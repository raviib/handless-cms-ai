...
import { DbValidator } from "@/app/utils/db/schema_validation/home-page.validation.js";
...
const FILE_PATH = "/file/home-page";
...
export async function GET(request) {
    try {
        ...
        let data = await home_pageSchema.findOne()
...
export async function PUT(request) {
    ...
    try {
        ...
        let record = await home_pageSchema.findOne();

        if (record) {
            await home_pageSchema.findByIdAndUpdate(
                record._id,
            ...
            await home_pageSchema.create({ ...objToPush });
            return NextResponse.json({
                success: true,
