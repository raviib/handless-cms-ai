...
export async function GET(request) {
    try {
        ...
        const totalCount = await jobs_openingSchema.countDocuments(query);
        
        // Get paginated data
        const data = await jobs_openingSchema.find(query)
          .select("_id displayName title location experience positions url")
          .sort({ sort: -1, name: 1 })
          .skip((page - 1) * limit)
          .limit(limit)
          .lean();
