...
import { DbValidator } from "@/app/utils/db/schema_validation/corporate-responsibility.validation.js";
...
const FILE_PATH = "/file/corporate-responsibility";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { corporate_responsibility_id } = await params;
        let data = await corporate_responsibilitySchema.findById(corporate_responsibility_id);
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await corporate_responsibilitySchema.findByIdAndUpdate(
            corporate_responsibility_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { corporate_responsibility_id } = await params;
        const data = await corporate_responsibilitySchema.findByIdAndDelete(corporate_responsibility_id);
