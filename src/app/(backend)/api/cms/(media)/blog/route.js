...
import { DbValidator } from "@/app/utils/db/schema_validation/blog.validation.js";
...
export async function GET(request) {
    try {
        ...
        let data = await blogSchema.find({ ...que })
            .select(fieldSelector)
            .skip((page - 1) * limit)
            .limit(limit)
            .sort({ sort: 1 })
            .populate({ path: "tag", select: "displayName _id" }); // Populate first-level relation fields
...
export async function POST(request) {
    ...
    try {
        ...
        await blogSchema.create({ ...objToPush });
