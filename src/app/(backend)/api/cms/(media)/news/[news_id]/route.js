...
import { DbValidator } from "@/app/utils/db/schema_validation/news.validation.js";
...
const FILE_PATH = "/file/news";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { news_id } = await params;
        let data = await newsSchema.findById(news_id);
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await newsSchema.findByIdAndUpdate(
            news_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { news_id } = await params;
        const data = await newsSchema.findByIdAndDelete(news_id);
