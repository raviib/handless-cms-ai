...
import { DbValidator } from "@/app/utils/db/schema_validation/blog-tag.validation.js";
...
const FILE_PATH = "/file/blog-tag";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { blog_tag_id } = await params;
        let data = await blog_tagSchema.findById(blog_tag_id); 
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await blog_tagSchema.findByIdAndUpdate(
            blog_tag_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { blog_tag_id } = await params;
        const data = await blog_tagSchema.findByIdAndDelete(blog_tag_id);
