...
import { DbValidator } from "@/app/utils/db/schema_validation/event.validation.js";
...
const FILE_PATH = "/file/event";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { event_id } = await params;
        let data = await eventSchema.findById(event_id);
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { event_id } = await params;
        const data = await eventSchema.findByIdAndDelete(event_id);
