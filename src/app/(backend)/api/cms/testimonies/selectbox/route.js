...
export async function GET(request) {
    try {
        ...
        const totalCount = await testimoniesSchema.countDocuments(query);
        const data = await testimoniesSchema.find(query)
          .select("_id displayName person_name company url")
          .sort({ sort: -1, name: 1 })
          .skip((page - 1) * limit)
          .limit(limit)
          .lean();
