...
import { DbValidator } from "@/app/utils/db/schema_validation/testimonies.validation.js";
...
const FILE_PATH = "/file/testimonies";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { testimonies_id } = await params;
        let data = await testimoniesSchema.findById(testimonies_id);
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await testimoniesSchema.findByIdAndUpdate(
            testimonies_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { testimonies_id } = await params;
        const data = await testimoniesSchema.findByIdAndDelete(testimonies_id);
