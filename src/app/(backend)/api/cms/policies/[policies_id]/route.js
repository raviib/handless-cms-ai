...
import { DbValidator } from "@/app/utils/db/schema_validation/policies.validation.js";
...
const FILE_PATH = "/file/policies";

export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { policies_id } = await params;
        let data = await policiesSchema.findById(policies_id); 
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await policiesSchema.findByIdAndUpdate(
            policies_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { policies_id } = await params;
        const data = await policiesSchema.findByIdAndDelete(policies_id);
