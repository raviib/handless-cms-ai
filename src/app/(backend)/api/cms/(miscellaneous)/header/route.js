import headerSchema from "@/app/(backend)/models/cms/(miscellaneous)/header/header.modal.js";
...
import { DbValidator } from "@/app/utils/db/schema_validation/header.validation.js";
...
export async function GET(request) {
    try {
        ...
        let data = await headerSchema.findOne()
...
export async function PUT(request) {
    ...
    try {
        ...
        let record = await headerSchema.findOne();

        if (record) {
            await headerSchema.findByIdAndUpdate(
                record._id,
            ...
            await headerSchema.create({ ...objToPush });
            return NextResponse.json({
                success: true,
