...
import { DbValidator } from "@/app/utils/db/schema_validation/footer.validation.js";
...
export async function GET(request) {
    try {
        ...
        let data = await footerSchema.findOne()
...
export async function PUT(request) {
    ...
    try {
        ...
        let record = await footerSchema.findOne();

        if (record) {
            await footerSchema.findByIdAndUpdate(
                record._id,
            ...
            await footerSchema.create({ ...objToPush });
            return NextResponse.json({
                success: true,
