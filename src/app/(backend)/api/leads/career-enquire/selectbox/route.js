...
export async function GET(request) {
    try {
        ...
        const totalCount = await career_enquireSchema.countDocuments(query);
        
        // Get paginated data
        const data = await career_enquireSchema.find(query)
          .select("_id displayName name phone subject")
          .sort({ sort: -1, name: 1 })
          .skip((page - 1) * limit)
          .limit(limit)
          .lean();
