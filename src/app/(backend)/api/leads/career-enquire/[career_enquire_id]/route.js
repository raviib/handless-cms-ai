...
export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { career_enquire_id } = await params;
        let data = await career_enquireSchema.findById(career_enquire_id)
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await career_enquireSchema.findByIdAndUpdate(
            career_enquire_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { career_enquire_id } = await params;
        const data = await career_enquireSchema.findByIdAndDelete(career_enquire_id);
