...
export async function GET(request, { params }) {
    try {
        await dbConnect();
        const { enquire_now_id } = await params;
        let data = await enquire_nowSchema.findById(enquire_now_id)
...
export async function PUT(request, { params }) {
    ...
    try {
        ...
        const data = await enquire_nowSchema.findByIdAndUpdate(
            enquire_now_id,
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { enquire_now_id } = await params;
        const data = await enquire_nowSchema.findByIdAndDelete(enquire_now_id);
