...
export async function GET(request) {
    try {
        ...
        const totalDocs = await brandsSchema.find({ ...que }).countDocuments();
        let data = await brandsSchema.find({ ...que })
            .select(fieldSelector)
            .skip((page - 1) * limit)
            .limit(limit)
            .sort({ sort: 1 })
            .populate({ path: "categorys", select: "name _id" }); // Populate first-level relation fields
...
export async function POST(request) {
    ...
    try {
        ...
        await brandsSchema.create({ ...objToPush });
