...
import { DbValidator } from "@/app/utils/db/schema_validation/products.validation.js";
...
export async function GET(request) {
    try {
        ...
        let data = await productsSchema.find({ ...que })
            .select(fieldSelector)
            .skip((page - 1) * limit)
            .limit(limit)
            .sort({ sort: 1 })
            .populate({ path: "category", select: "name _id" })
            .populate({ path: "catalogue", select: "displayName _id" }); // Populate first-level relation fields
...
export async function POST(request) {
    ...
    try {
        ...
        await productsSchema.create({ ...objToPush });
