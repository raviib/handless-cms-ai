...
export async function GET(request, { params }) {
    try {
        ...
        let data = await productsSchema.findById(products_id)
            .populate({ path: "category", select: "name _id" })
            .populate({ path: "catalogue", select: "displayName _id" });
...
export async function DELETE(request, { params }) {
    try {
        await dbConnect();
        const { products_id } = await params;
        const data = await productsSchema.findByIdAndDelete(products_id);
