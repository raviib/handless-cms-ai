                                ...
                                <div className='flex with-tooltip'>
                                    ...
                                    {subField.type === "media" && !subField.isMulti && (
                                        <AiImageSuggestButton
                                            fieldId={[recordId, locale !== "en" ? locale : "", moduleSlug, fieldKey, subFieldKey].filter(Boolean).join(".")}
                                            contextText={Object.values(componentData).filter(v => typeof v === "string" && v.trim()).slice(0, 3).join(" ").slice(0, 400)}
                                            onApply={(imagePath) => handleFieldChange({ target: { name: subFieldKey, value: imagePath } }, subFieldKey, null)}
                                        />
                                    )}
                                    {subField.type === "media" && subField.isMulti && (
                                        <AiMultiImageSuggestButton
                                            ...
                                            contextText={Object.values(componentData).filter(v => typeof v === "string" && v.trim()).slice(0, 3).join(" ").slice(0, 400)}
                                            onApply={(imagePaths) => {
                                                const existing = Array.isArray(componentData[subFieldKey]) ? componentData[subFieldKey] : [];
                                                handleFieldChange([...existing, ...imagePaths], subFieldKey, "file");
                                            }}
                                        />
                                    )}
                                </div>
